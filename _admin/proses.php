...
elseif (isset($_POST['keterlambatan_siswa'])) {
    ...
    // Asumsi: ID guru yang login tersimpan di $data['id_guru']
    $id_guru_piket  = $data['id_guru'] ?? null; 
    ...
    $nama_guru_piket = $data['nama'] ?? 'Guru Piket'; // Gunakan nama untuk pesan
...
elseif (isset($_POST['e_izin_siswa'])) {
    ...
    // 2. Ambil ID Guru Piket yang memproses
    // Asumsi: ID guru yang login tersimpan di $data['id_guru']
    $id_guru_piket  = $data['id_guru'] ?? null; 

    // Tentukan nilai SQL untuk id_guru_piket (tidak boleh kosong jika status diubah dari Menunggu)
    $id_guru_piket_sql = is_null($id_guru_piket) ? 'NULL' : "'".mysqli_real_escape_string($con, $id_guru_piket)."'";
    ...
    if (!empty($catatan_proses)) {
        // Format catatan baru
        $nama_pemroses = $data['nama'] ?? 'Guru/Admin';
    ...
    $query_update_izin = "
        ...
        SET
            ...
            id_guru_piket = $id_guru_piket_sql,
    ...
    $nama_guru_piket = $data['nama'] ?? 'Admin';
    echo " <script>alert('Status Izin Siswa Berhasil Diperbarui menjadi $status_izin oleh $nama_guru_piket.');window.location='?page=v_izin_siswa';</script> ";
}
?>